...
void MainWindow::OnPaint() {
    ...
    if (m_updateChecked) {
        if (m_updateAvailable) {
            StringCchPrintfW(versionFull, ARRAYSIZE(versionFull), L"Update Available | v1.2.6");
        } else {
            StringCchPrintfW(versionFull, ARRAYSIZE(versionFull), L"Up to Date | v1.2.6");
        }
    } else {
        StringCchCopyW(versionFull, ARRAYSIZE(versionFull), L"v1.2.6");
    }
